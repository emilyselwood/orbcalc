...
function init() {
  ...
  // Modules don't play nice with on attributes so we have to do this
  const tourButton = document.getElementById("tourbutton");
  tourButton.addEventListener("click", toggleTour, false);
...
function loadData(name, mat, color, T, raytarget) {
  ...
  loader.load(
    ...
    function (data) {
      ...
      scene.add(points);
      console.log("loaded " + name + " with " + positions.length);
    },
...
function setupMove() {
  ...
  tween = new JEASINGS.JEasing(startPos)
    .to(targetPos, 10000)
